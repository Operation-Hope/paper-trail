...
export function applyChartJSTheme(theme: Theme = 'light') {
  ...
  if (ChartJS.defaults.plugins.tooltip) {
    ...
    ChartJS.defaults.plugins.tooltip.backgroundColor = theme === 'dark'
      ? 'rgba(31, 41, 55, 0.95)'
      : 'rgba(17, 24, 39, 0.9)';
    ChartJS.defaults.plugins.tooltip.padding = 12;
    ChartJS.defaults.plugins.tooltip.cornerRadius = 6;
    ChartJS.defaults.plugins.tooltip.titleColor = theme === 'dark' ? '#f3f4f6' : '#ffffff';
    ChartJS.defaults.plugins.tooltip.bodyColor = theme === 'dark' ? '#e5e7eb' : '#ffffff';
  }
}
