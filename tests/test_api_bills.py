# ABOUTME: Tests for bills API endpoints
# ABOUTME: Tests the bill subjects endpoint for data integrity and error handling
...
class TestBillSubjects:
    ...
    def test_subjects_not_empty(self, client):
        """Subjects list is not empty (database has data)."""
        response = client.get('/api/bills/subjects')
        assert response.status_code == 200
        data = json.loads(response.data)

        # If there are bills with subjects, list should not be empty
        # This test may fail if database is empty, which is acceptable
        if len(data) > 0:
            assert len(data) > 0
